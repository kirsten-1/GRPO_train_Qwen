...
def reasoning_quality_score(reasoning: str) -> float:
    ...
    has_steps = 1.0 if re.search(r"(1[\.\、]|步骤|首先|然后|最后|解题思路)", reasoning) else 0.0
    has_verify = 1.0 if re.search(r"(验证|检查|合理性|test|assert)", reasoning, flags=re.IGNORECASE) else 0.0
...
class LocalChatGenerator:
    ...
    def generate(
        ...
        attention_mask = torch.ones_like(prompt_ids, device=self.model.device)
...
def load_processed_samples(dataset_name: str, max_samples: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    if dataset_name == "math":
        raw = load_dataset("EleutherAI/hendrycks_math", "algebra", cache_dir=DATASETS_CACHE_DIR)["test"]
        if len(raw) == len(samples):
            for i in range(len(samples)):
                samples[i]["difficulty"] = raw[i].get("level", "unknown")
    if dataset_name == "cmath":
        raw = load_dataset("weitianwen/cmath", cache_dir=DATASETS_CACHE_DIR)["test"]
        if len(raw) == len(samples):
            for i in range(len(samples)):
    ...
    for sample in pbar:
        ...
        for out in outs:
            ...
            if dataset_name == "mbpp":
                ...
                io_pairs = load_apps_io_pairs(sample["io_path"], max_cases=apps_max_cases)
                if not io_pairs:
                    ...
                    if fn_name:
                        ...
                        r = eval_apps_code_stdio(code, io_pairs=io_pairs, timeout_s=timeout_s, max_cases=apps_max_cases)
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--code-num-samples", type=int, default=10, help="用于 Pass@k 的采样数量（建议 >=10）")
